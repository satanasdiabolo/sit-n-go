...
class MatchService
{
	...
	function updatePlayerRank($login, $matchId, $rank)
	{
		if(!$this->isPlayerConfirmed($login, $matchId))
		{
			throw new Exception('Player is not register in this match');
		}
		$this->db->execute(
			'UPDATE SitnGo_Players SET rank = %d WHERE login = %s AND matchId = %d',
			$rank, $this->db->quote($login), $matchId
		);
